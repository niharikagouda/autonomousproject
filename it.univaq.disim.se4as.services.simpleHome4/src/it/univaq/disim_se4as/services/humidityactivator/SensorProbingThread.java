			...
			for (int i = 0; i < refs.length; i++) {
				...
				if (refs[i] != null) {
					if (sensor != null) {
						...
						int hum = sensor.getData().getVal();

						for (int k = 0; k < 8; k++) {
							connection.send(Topics.WEATHER_HUMIDITY.getTopic(), (hum + 65 + k) + "");
							System.out.println("Humidity Sensed value: " + (hum + 65 + k) + sensor.getData().getUnit());
							Thread.sleep(5000);
